...
struct _BufferProjection:
    ...
    def to_json(
        ...
        if len(source) > 1048576:
            raise Error(
                "Buffer JSON presentation exceeds its value-node budget"
            )
        var numbers = List[JsValue](capacity=len(source))
        for index in range(len(source)):
            numbers.append(
                JsValue(Float64(source._bytes[][source._offset + index]))
            )
    ...
    def to_string(
        ...
        var source = context.unsafe_bitcast[Self]()[].source
        var bytes = Span(source._bytes[])[
            source._offset : source._offset + source._length
        ]
    ...
    def inspect(
        ...
        for index in range(min(50, len(source))):
            ...
            output += (
                " "
                + String(digits[byte=byte >> 4])
                + String(digits[byte=byte & 15])
            )
        if len(source) > 50:
            ...
            output += (
                " ... "
                + String(remaining)
                + (" more byte" if remaining == 1 else " more bytes")
            )
        return output + ">"
...
def buffer_to_js_value(source: Buffer) -> JsValue:
    var environment = allocate_callable_environment(
        _BufferProjection(source),
        destroy_callable_environment[_BufferProjection],
    )
    return js_value_from_native_bytes(
        JsByteView(
            source._bytes, source._offset, source._length, source._identity
        ),
        buffer_brand,
        RaisingCallable[Tuple[String], JsValue, Error](
            environment, _BufferProjection.to_json
        ),
