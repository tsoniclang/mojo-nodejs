...
struct _ChannelOwner(Movable):
    ...
    def __deinit__(deinit self):
        if self.handle:
            external_call["tsonic_node_worker_free", NoneType](
                self.handle.value()
            )
...
struct WorkerChannel(ImplicitlyCopyable):
    ...
    def close(self):
        external_call["tsonic_node_worker_close", NoneType](
            self._owner[].handle.value()
        )

    def closed(self) -> Bool:
        return (
            external_call["tsonic_node_worker_closed", c_int](
                self._owner[].handle.value()
            )
            != 0
        )
    ...
    def progress(self) raises:
        _check(
            external_call["tsonic_node_worker_progress", c_int](
                self._owner[].handle.value()
            )
        )
    ...
    def pending(self) -> Bool:
        return (
            external_call["tsonic_node_worker_pending", c_int](
                self._owner[].handle.value()
            )
            != 0
        )
    ...
    def send(self, kind: UInt8, bytes: List[UInt8]) raises:
        _check(
            external_call["tsonic_node_worker_send", c_int](
                self._owner[].handle.value(),
                kind,
                bytes.unsafe_ptr(),
                c_size_t(len(bytes)),
    ...
    def receive(self) raises -> Optional[NativeMessage]:
        ...
        external_call["tsonic_node_worker_consume", NoneType](
            self._owner[].handle.value()
        )
    ...
    def wait_message(self, timeout_ms: Int32 = 30000) raises:
        _check(
            external_call["tsonic_node_worker_wait", c_int](
                self._owner[].handle.value(), timeout_ms
            )
        )

    def flush(self, timeout_ms: Int32 = 30000) raises:
        _check(
            external_call["tsonic_node_worker_flush", c_int](
                self._owner[].handle.value(), timeout_ms
            )
        )
    ...
    def exit_code(self) raises -> Optional[Int32]:
        var code = Int32(0)
        var status = external_call["tsonic_node_worker_exit", c_int](
            self._owner[].handle.value(), Pointer(to=code)
        )
        if status < 0:
            _check(-status)
    ...
    def id(self) -> Float64:
        return Float64(
            external_call["tsonic_node_worker_id", c_int](
                self._owner[].handle.value()
            )
        )
    ...
    def terminate(self) raises:
        _check(
            external_call["tsonic_node_worker_terminate", c_int](
                self._owner[].handle.value()
            )
        )
...
def new_pair() raises -> Tuple[WorkerChannel, WorkerChannel]:
    ...
    _check(
        external_call["tsonic_node_worker_pair", c_int](
            Pointer(to=first), Pointer(to=second)
        )
    )
