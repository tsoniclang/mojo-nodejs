...
from .ports import MessagePort, Listener0, Listener1, register_port, poll_worker_threads, INITIALIZE
    ...
    async def terminate(self) raises -> Float64:
        ...
        while not self._port._state[].exited:
            ...
            if monotonic_milliseconds() >= deadline:
                raise Error("Worker termination did not complete within its resource deadline")
            if not self._port._state[].exited:
                sleep(0.001)
        return Float64(self._port._state[].exit_code.value())

    def thread_id(self) -> Float64:
        return -1 if self._port._state[].exited else self._port._state[].channel.id()
...
def worker_new(identity: String, options: WorkerOptions = WorkerOptions()) raises -> Worker:
    var name = options.name.value() if options.name else String()
    if name.find("\0") >= 0:
        raise Error("Worker name contains a null byte")
    var initialization = js_value_from_array_values(List[JsValue](
        JsValue(JsString(identity)), options.worker_data, environment_snapshot(), JsValue(JsString(name)),
    ))
    ...
    var channel = spawn_channel(arguments, environment, options.env.is_undefined())
