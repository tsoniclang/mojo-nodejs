...
from .core import MkdirOptions, make_directory, read_directory_names, copy_file, unlink, symbolic_link
...
def _within(source: String, destination: String) raises -> Bool:
    ...
    return child == parent or child.startswith(parent + ("" if parent.endswith("/") else "/"))


def _same(left: Stats, right: Stats) -> Bool:
    return left.inode != 0 and left.device != 0 and left.inode == right.inode and left.device == right.device
...
struct CopyTraversal:
    ...
    def __init__(out self, source: String, destination: String, options: CopyOptions, asynchronous: Bool = False) raises:
        checked_path(source)
        ...
        self._mode = Float64(checked_integer(options.mode.value(), 7, "copy mode")) if options.mode else 0
    ...
    def accept(mut self, entry: CopyEntry) raises:
        var follow = self._options.dereference.value() if self._options.dereference else False
        ...
        if destination:
            ...
            if source.is_directory() != destination.value().is_directory():
                raise Error("cp: incompatible directory and non-directory operands")
        if source.is_directory():
            if not self._options.recursive or not self._options.recursive.value():
            ...
            for ancestor in self._ancestors:
                if _same(source, ancestor):
                    raise Error("cp: ELOOP: symbolic link creates a directory cycle")
            ...
            while True:
                ...
                if parent_stat and _same(source, parent_stat.value()):
                    raise Error("cp: EINVAL: destination parent aliases the source")
        ...
        elif source.is_file() or (source.mode & 0o170000) == 0o020000 or (source.mode & 0o170000) == 0o060000:
            self._file(entry, source, destination)
        else:
            raise Error("cp: EINVAL: unsupported filesystem object")

    def _directory(mut self, entry: CopyEntry, source: Stats, destination: Optional[Stats]) raises:
        if not destination:
            make_directory(entry.destination)
        elif self._async and self._options.error_on_exist and self._options.error_on_exist.value() and self._options.force and not self._options.force.value():
            raise Error("cp: EEXIST: destination directory exists")
        self._ancestors.append(source.copy())
        self._pending.append(CopyEntry(entry.source, entry.destination, source.mode if not destination else -1))
        var names = read_directory_names(entry.source)
        for index in range(len(names) - 1, -1, -1):
            self._pending.append(CopyEntry(String(Path(entry.source) / names[index]), String(Path(entry.destination) / names[index]), None))

    def _file(self, entry: CopyEntry, source: Stats, destination: Optional[Stats]) raises:
        if destination:
            var force = self._options.force.value() if self._options.force else True
            if not force:
                if self._options.error_on_exist and self._options.error_on_exist.value():
                    raise Error("cp: EEXIST: destination exists")
                return
            unlink(entry.destination)
        copy_file(entry.source, entry.destination, self._mode)
        if self._options.preserve_timestamps and self._options.preserve_timestamps.value():
            ...
            check_status(external_call["tsonic_node_fs_utimes", Int32](destination_path.as_c_string_slice().ptr(), updated.atime_ms / 1000, updated.mtime_ms / 1000), "cp.utimes")
    ...
    def _link(self, entry: CopyEntry, destination: Optional[Stats]) raises:
        ...
        var verbatim = self._options.verbatim_symlinks.value() if self._options.verbatim_symlinks else False
        ...
        if destination:
            ...
            if not is_absolute(previous):
                previous = _absolute(String(Path(dirname(entry.destination)) / previous))
            var source_stat = stat_if_present(entry.source)
            if source_stat and source_stat.value().is_directory() and _within(target, previous):
...
def copy_tree(source: String, destination: String, options: CopyOptions = CopyOptions()) raises:
    ...
    try:
        while True:
            ...
            if not entry:
                return
            if options.filter and not options.filter.value().call((entry.value().source, entry.value().destination)):
                continue
