...
def open_file(
    path: String, flags: String, mode: Float64 = 0o666
) raises -> Float64:
    checked_path(path)
    checked_path(flags)
    ...
    var descriptor = external_call["tsonic_node_fs_open", c_int](
        ...
        native_flags.as_c_string_slice(),
        permissions,
    )
    if descriptor < 0:
...
def _position(value: Optional[Float64]) raises -> Optional[Int64]:
    return Optional(
        checked_integer(value.value(), 9007199254740991, "position")
    ) if value else None
...
def read_into(
    ...
    offset: Float64,
    length: Float64,
    position: Optional[Float64] = None,
) raises -> Float64:
    var number = c_int(checked_integer(descriptor, 2147483647, "descriptor"))
    var first = Int(
        checked_integer(offset, Float64(len(buffer)), "buffer offset")
    )
    var count = Int(
        checked_integer(length, Float64(len(buffer) - first), "buffer length")
    )
    ...
    var read = external_call["tsonic_node_stream_read", Int64](
        number,
        buffer._bytes[].unsafe_ptr().unsafe_offset(buffer._offset + first),
        c_size_t(count),
        location.value() if location else Int64(0),
        c_int(Bool(location)),
    )
    if read < 0:
...
def write_from(
    ...
    var count = (
        Int(
            checked_integer(
                length.value(), Float64(len(buffer) - first), "buffer length"
            )
        ) if length else len(buffer)
        - first
    )
    var location = _position(position)
    if count == 0:
...
def write_string(
    ...
    return write_from(
        descriptor,
        buffer_from_string_encoded(value, encoding),
        0,
        None,
        position,
    )
...
def access(path: String, mode: Float64 = 0) raises:
    ...
    check_status(
        external_call["tsonic_node_fs_access", Int32](
            native_path.as_c_string_slice(), permissions
        ),
        "access",
    )
