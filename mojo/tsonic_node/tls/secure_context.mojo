...
struct SecureContextOptions(Copyable):
    ...
    def __init__(out self, key: Optional[String] = None, cert: Optional[String] = None,
        var ca: Optional[List[String]] = None, pfx: Optional[Buffer] = None,
        passphrase: Optional[String] = None, min_version: Optional[String] = None,
...
def create_secure_context(options: SecureContextOptions = SecureContextOptions()) raises -> SecureContext:
    ...
    var handle = external_call["tsonic_node_tls_context_create", OptionalPointer[NoneType, MutUntrackedOrigin]](
        key_pointer, cert_pointer, ca.as_c_string_slice().ptr().as_unsafe_any_origin(),
        c_int(Bool(options.ca)), pfx.unsafe_ptr(), c_size_t(len(pfx)), c_int(Bool(options.pfx)),
        password.as_c_string_slice().ptr().as_unsafe_any_origin(),
        tls_version(options.min_version), tls_version(options.max_version), Pointer(to=error),
    )
    if not handle:
