...
struct URLSearchParams(Equatable, ImplicitlyCopyable, Sized):
    ...
    def __len__(self) -> Int:
        return Int(
            external_call["tsonic_node_url_params_size", c_size_t](
    ...
    def _has(self, name: String, value: String, match_value: Bool) -> Bool:
        return (
            external_call["tsonic_node_url_params_has", c_int](
                self._state[].handle.value(),
    ...
    def _mutate(self, operation: Int32, name: String, value: String) raises:
        if (
            external_call["tsonic_node_url_params_mutate", c_int](
                self._state[].handle.value(),
                c_int(operation),
                name.as_bytes().unsafe_ptr(),
                c_size_t(name.byte_length()),
    ...
    def _at(self, index: Int, key: Bool) -> String:
        ...
        return String(
            unsafe_from_utf8=Span(
                unsafe_ptr=text.value().unsafe_bitcast[Byte](),
                length=Int(length),
            )
        )
