...
def create_url_state(
    input: String, base: Optional[String]
) raises -> ArcPointer[UrlState]:
    var base_text = base.value() if base else String()
    var handle = external_call[
        "tsonic_node_url_new", OptionalPointer[NoneType, MutUntrackedOrigin]
    ](
...
def get_url_field(state: ArcPointer[UrlState], field: Int32) raises -> String:
    ...
    var text = external_call[
        "tsonic_node_url_get", OptionalPointer[c_char, ImmUntrackedOrigin]
    ](
        state[].handle.value(),
        c_int(field),
        Pointer(to=length),
    ...
    return String(
        unsafe_from_utf8=Span(
            unsafe_ptr=text.value().unsafe_bitcast[Byte](), length=Int(length)
        )
    )


def set_url_field(
    state: ArcPointer[UrlState], field: Int32, value: String
) raises:
    if (
        external_call["tsonic_node_url_set", c_int](
            state[].handle.value(),
            c_int(field),
            value.as_bytes().unsafe_ptr(),
            c_size_t(value.byte_length()),
        )
        != 0
    ):
