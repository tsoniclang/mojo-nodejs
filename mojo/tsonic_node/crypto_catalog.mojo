...
def _names(kind: Int32) raises -> List[String]:
    var names = external_call[
        "tsonic_node_crypto_names",
        OptionalPointer[NoneType, MutUntrackedOrigin],
    ](c_int(kind))
    if not names:
        raise Error(
            "Unable to enumerate cryptographic algorithms: ", get_errno()
        )
    try:
        var size = Int(
            external_call["tsonic_node_crypto_names_size", c_size_t](
                names.value()
            )
        )
        ...
        for index in range(size):
            var name = external_call[
                "tsonic_node_crypto_name_at",
                OptionalPointer[c_char, ImmUntrackedOrigin],
            ](names.value(), c_size_t(index))
            if not name:
                raise Error(
                    "Cryptographic algorithm inventory changed during"
                    " enumeration"
                )
