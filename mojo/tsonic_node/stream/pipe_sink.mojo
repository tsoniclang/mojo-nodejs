...
struct PipeSink(ImplicitlyCopyable):
    ...
    def ready(self) -> Bool:
        if self.state[].waiting and self.state[].target.isa[Writable]():
            self.state[].waiting = (
                self.state[].target.unsafe_get[Writable]().pipe_needs_drain()
            )
    ...
    def write(mut self, value: StreamChunk) raises:
        ...
        if self.state[].target.isa[Writable]():
            self.state[].waiting = (
                not self.state[]
                .target.unsafe_get[Writable]()
                .write_value(value)
            )
        elif value.isa[Buffer]():
            _ = (
                self.state[]
                .target.unsafe_get[ServerResponse]()
                .write_buffer(value.unsafe_get[Buffer]())
            )
        else:
            _ = (
                self.state[]
                .target.unsafe_get[ServerResponse]()
                .write_buffer(Buffer.from_string(value.unsafe_get[String]()))
            )
...
struct PipeSubscription(ImplicitlyCopyable):
    ...
    def __init__(out self, sink: PipeSink):
        ...
        var environment = allocate_callable_environment(
            sink, destroy_callable_environment[PipeSink]
        )
