...
struct TypedListeners[Arguments: AnyType](ImplicitlyCopyable):
    ...
    def __init__(out self) where conforms_to(Self.Arguments, Movable & Deinitable):
        self._state = ArcPointer(_Listeners[Self.Payload](List[_Listener[Self.Payload]](), 0))

    def has_listeners(self) -> Bool:
        return len(self._state[].entries) != 0

    def add(self, callback: RaisingCallable[Self.Arguments, NoneType], once: Bool = False) raises where conforms_to(Self.Arguments, Movable & Deinitable):
        ...
        self._state[].entries.append(_Listener[Self.Payload](self._state[].next_identity,
            rebind_var[RaisingCallable[Self.Payload, NoneType]](callback), fired))

    def remove(self, callback: RaisingCallable[Self.Arguments, NoneType]) where conforms_to(Self.Arguments, Movable & Deinitable):
        var found = -1
        var selected = rebind_var[RaisingCallable[Self.Payload, NoneType]](callback)
    ...
    def emit(self, arguments: Self.Arguments) raises -> Bool where conforms_to(Self.Arguments, Copyable & Deinitable):
