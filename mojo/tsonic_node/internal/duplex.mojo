...
struct Duplex(ImplicitlyCopyable):
    ...
    def __init__(
        ...
        write: def(
            ErasedCallableContext, var Tuple[Buffer]
        ) thin raises -> Bool,
        end: def(
...
struct _DuplexAdapter[Stream: ImplicitlyCopyable & Deinitable]:
    ...
    @staticmethod
    def write_value(
        context: ErasedCallableContext, var arguments: Tuple[Buffer]
    ) raises -> Bool:
        var owner = context.unsafe_bitcast[Self]()
    ...
    @staticmethod
    def end_value(
        context: ErasedCallableContext, var arguments: Tuple[Optional[Buffer]]
    ) raises:
        var owner = context.unsafe_bitcast[Self]()
        var end = owner[].end
