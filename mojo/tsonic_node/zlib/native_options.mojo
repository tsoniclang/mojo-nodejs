...
def create_native(
    mode: Int32, options: CodecOptions
) raises -> OptionalPointer[NoneType, MutUntrackedOrigin]:
...
def create_native(
    mode: Int32, options: ZlibOptions
) raises -> OptionalPointer[NoneType, MutUntrackedOrigin]:
    ...
    var handle = external_call[
        "tsonic_node_codec_zlib_new",
        OptionalPointer[NoneType, MutUntrackedOrigin],
    ](
        c_int(mode),
        c_int(optional_integer(options.level)),
        c_int(optional_integer(options.window_bits)),
        c_int(optional_integer(options.mem_level)),
        c_int(optional_integer(options.strategy)),
        dictionary.unsafe_ptr(),
        c_size_t(dictionary_length),
        c_size_t(chunk_size(options.chunk_size)),
        c_size_t(output_limit(options.max_output_length)),
        Pointer(to=error),
    )
    if not handle:
...
def create_native(
    mode: Int32, options: BrotliOptions
) raises -> OptionalPointer[NoneType, MutUntrackedOrigin]:
    ...
    if options.params:
        for entry in options.params.value().items():
            ...
            values.append(
                UInt32(
                    checked_integer(
                        numeric, 4294967295, "Brotli parameter value"
                    )
                )
            )
    ...
    var error = OptionalPointer[UInt8, MutUntrackedOrigin]()
    var handle = external_call[
        "tsonic_node_codec_brotli_new",
        OptionalPointer[NoneType, MutUntrackedOrigin],
    ](
        c_int(mode),
        identifiers.unsafe_ptr(),
        values.unsafe_ptr(),
        c_size_t(count),
        c_size_t(chunk_size(options.chunk_size)),
        c_size_t(output_limit(options.max_output_length)),
