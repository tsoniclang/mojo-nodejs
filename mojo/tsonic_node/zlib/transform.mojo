...
from .options import ZlibOptions, BrotliOptions, CodecOptions, MAX_OUTPUT, optional_integer, finish_flush, write_flush
    ...
    def __init__(out self, mode: Int32, var options: CodecOptions) raises:
        ...
        if options.isa[ZlibOptions]():
            ...
            if selected.dictionary:
                selected.dictionary = Buffer(selected.dictionary.value().copy_bytes())
...
struct Zlib(ImplicitlyCopyable):
    ...
    def __init__(out self, mode: Int32, options: ZlibOptions = ZlibOptions()) raises:
    ...
    def write(self, input: Buffer) raises -> Bool:
        self._require_open()
        self._retain(self._state[].codec.write(input, write_flush(self._state[].options)))
        return True
    ...
    def read(self) raises -> Optional[Buffer]:
        if self._state[].output_bytes == 0:
            return None
        var result = self._state[].output[0] if len(self._state[].output) == 1 else buffer_concat(self._state[].output)
    ...
    def end_buffer(self, input: Buffer) raises:
        ...
        self._retain(self._state[].codec.write(input, finish_flush(self._state[].options)))
    ...
    def flush_kind(self, kind: Float64) raises:
        self._require_open()
        var selected = checked_integer(kind, 5.0 if self._state[].mode < 8 else 2.0, "compression flush")
    ...
    def params(self, level: Float64, strategy: Float64) raises:
        self._require_open()
        self._retain(self._state[].codec.params(optional_integer(level), optional_integer(strategy)))
        var options = self._state[].options.unsafe_get[ZlibOptions]()
        options.level = level
        options.strategy = strategy
        self._state[].options = CodecOptions(options)

    def params_callback(self, level: Float64, strategy: Float64, callback: Notification) raises:
        self.params(level, strategy)
        defer_notification(callback)

    def reset(self) raises:
        self._require_open()
        self._state[].codec = NativeCodec(self._state[].mode, self._state[].options)
    ...
    def _require_open(self) raises:
        if self._state[].ended or self._state[].destroyed or not self._state[].codec.handle:
    ...
    def _retain(self, output: Buffer) raises:
        if len(output) > MAX_OUTPUT - self._state[].output_bytes:
            self.destroy()
            raise Error("Pending compression output exceeds the finite runtime limit")
        if len(output) != 0:
