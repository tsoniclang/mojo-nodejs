...
def process_buffer(input: Buffer, mode: Int32, options: CodecOptions) raises -> Buffer:
    if wants_info(options):
        raise Error("Compression info requires the engine/result contract")
    return process(input, mode, options)


def process_info(input: Buffer, mode: Int32, options: CodecOptions) raises -> ZlibInfo:
    if not wants_info(options):
        raise Error("Compression info result requires info: true")
    var engine = Zlib(mode, options.unsafe_get[ZlibOptions]()) if options.isa[ZlibOptions]() else Zlib(mode, options.unsafe_get[BrotliOptions]())
    engine.end_buffer(input)
    return ZlibInfo(engine.read().value_or(Buffer()), engine)


def process_result(input: Buffer, mode: Int32, options: CodecOptions) raises -> ZlibResult:
    if wants_info(options):
