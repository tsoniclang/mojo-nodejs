...
struct _NativeRequestState(Movable):
    ...
    def __deinit__(deinit self):
        if self.handle:
            external_call["tsonic_node_http_free", NoneType](
                self.handle.value()
            )
...
struct NativeRequest(ImplicitlyCopyable):
    ...
    def __init__(out self, url: String, options: RequestOptions) raises:
        ...
        var handle = external_call[
            ...
            native_url.as_c_string_slice().unsafe_ptr(),
        ...
        if (
            external_call["tsonic_node_http_tls", c_int](
                ...
                pfx.unsafe_ptr(),
                c_size_t(len(pfx)),
                password.as_c_string_slice().unsafe_ptr(),
            )
            == 0
        ):
            self.check_error()
    ...
    def complete(self) -> Bool:
        return (
            ...
            and external_call["tsonic_node_http_complete", c_int](
    ...
    def check_error(self) raises:
        var message = external_call[
            "tsonic_node_http_error", OptionalPointer[UInt8, ImmUntrackedOrigin]
        ](self._state[].handle.value())
        if message:
    ...
    def header(self, name: String, value: String) raises:
        ...
        if (
            external_call["tsonic_node_http_header", c_int](
                self._state[].handle.value(),
                line.as_c_string_slice().unsafe_ptr(),
            )
            == 0
        ):
    ...
    def start(
        ...
        var delay = c_long(
            checked_integer(timeout.value(), 2147483647, "request timeout")
        ) if timeout else c_long(0)
        var native_method = method
        if (
            external_call["tsonic_node_http_start", c_int](
                self._state[].handle.value(),
                native_method.as_c_string_slice().unsafe_ptr(),
                body.unsafe_ptr(),
                c_size_t(len(body)),
                c_int(present),
                delay,
    ...
    def status(self) -> Int32:
        return Int32(
            external_call["tsonic_node_http_status", c_int](
                self._state[].handle.value()
            )
        )
    ...
    def body(self) -> Buffer:
        ...
        var pointer = external_call[
            "tsonic_node_http_body", OptionalPointer[Byte, ImmUntrackedOrigin]
        ](
            self._state[].handle.value(),
            Pointer(to=length),
        )
