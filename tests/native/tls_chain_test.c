...
int main(void) {
    ...
    assert(error == NULL);
    assert(tsonic_tls_apply_certificate(server, chain, key_pem, &error));
    assert(error == NULL);
    ...
    assert(tsonic_tls_apply_certificate(server, leaf_pem, key_pem, &error));
    assert(!handshake(server, client));
    assert(!tsonic_tls_apply_certificate(server, malformed_chain, key_pem, &error));
    assert(error != NULL);
    free(error);
    error = NULL;
    assert(!tsonic_tls_apply_certificate(server, chain, wrong_key, &error));
    assert(error != NULL);
    free(error);
    error = NULL;
    assert(!tsonic_tls_apply_certificate(server, chain, encrypted_key, &error));
    assert(error != NULL);
    free(error);
    SSL_CTX_free(server);
    SSL_CTX_free(client);
