...
static void duplex_failure_proof(void) {
    TsonicWorkerChannel *first;
    TsonicWorkerChannel *second;
    assert(tsonic_node_worker_pair(&first, &second) == 0);
    uint8_t payload = 42;
    assert(tsonic_node_worker_send(second, 2, &payload, 1) == 0);
    payload = 43;
    assert(tsonic_node_worker_send(second, 2, &payload, 1) == 0);
    assert(tsonic_node_worker_flush(second, 10000) == 0);
    tsonic_node_worker_close(second);
    assert(tsonic_node_worker_send(first, 2, &payload, 1) == EPIPE);
    assert(!tsonic_node_worker_pending(first));
    assert(tsonic_node_worker_flush(first, 10000) == EPIPE);
    for (int index = 0; index < 2; index++) {
        assert(tsonic_node_worker_progress(first) == 0);
        uint8_t kind;
        const uint8_t *bytes;
        size_t size;
        assert(!tsonic_node_worker_closed(first));
        assert(tsonic_node_worker_message(first, &kind, &bytes, &size) == 1);
        assert(kind == 2 && size == 1 && bytes[0] == 42 + index);
        tsonic_node_worker_consume(first);
    }
    assert(tsonic_node_worker_progress(first) == 0);
    assert(tsonic_node_worker_closed(first));
