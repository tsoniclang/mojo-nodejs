...
from tsonic_node.os_info import (
    available_parallelism, dev_null, end_of_line, endianness, free_memory,
    load_average, machine, release, system_type, total_memory, uptime, version,
)
from tsonic_node.system_errors import get_system_error_message, get_system_error_name


def main() raises:
    assert_true(available_parallelism() >= 1)
    assert_equal(total_memory(), Float64(external_call["uv_get_total_memory", UInt64]()))
