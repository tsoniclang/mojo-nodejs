...
def require_buffer(value: Optional[StreamChunk]) raises -> Buffer:
    ...
    assert_true(value.value().isa[Buffer](), "A binary stream unexpectedly returned decoded text")
...
def require_text(value: Optional[StreamChunk]) raises -> String:
    ...
    assert_true(value.value().isa[String](), "A decoded stream unexpectedly returned a binary Buffer")
