...
from tsonic_node.worker_threads import MessageChannel, message_channel_new, receive_message_on_port, poll_worker_threads, mark_as_untransferable, is_marked_as_untransferable
...
struct Completion:
    ...
    def invoke(context: ErasedCallableContext, var arguments: Tuple[JsValue]) raises:
        ...
        if not environment[].created.read():
            ...
            _ = channel.port2.on_callable1(JsValue(JsString("message")), callback(environment[].count, environment[].created))
...
def callback(count: Location[Int], created: Location[Optional[MessageChannel]], fail: Bool = False) -> RaisingCallable[Tuple[JsValue], NoneType]:
    var environment = allocate_callable_environment(Completion(count, created, fail), Completion.destroy)
    return RaisingCallable[Tuple[JsValue], NoneType](environment, Completion.invoke)
...
def main() raises:
    ...
    var created = Location(Optional[MessageChannel]())
    _ = channel.port2.on_callable1(JsValue(JsString("message")), callback(count, created))
    ...
    _ = failing.port2.on_callable1(JsValue(JsString("message")), callback(count, created, True))
    _ = sibling.port2.on_callable1(JsValue(JsString("message")), callback(count, created))
    ...
    try:
        ...
        rejected = String(error).find("deliberate message callback failure") >= 0
