...
from tsonic_js import JsString, JsSymbol, JsValue, js_value_from_array_values, js_value_from_object_entries
from tsonic_js.value import JsValueWeakIdentity
from tsonic_node.worker_threads import get_environment_data, set_environment_data, is_marked_as_untransferable, mark_as_untransferable
...
def main() raises:
    ...
    assert_true(get_environment_data(JsValue(Float64(FloatLiteral.nan))).same_identity(value))
    ...
    assert_true(get_environment_data(JsValue(JsSymbol(JsString("key")))).is_undefined())
