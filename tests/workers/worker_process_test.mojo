...
def listener(
    count: Location[Int], code: Location[Float64], exit: Bool = False
) -> RaisingCallable[Tuple[JsValue], NoneType]:
    var environment = allocate_callable_environment(
        CountMessage(count, code, exit), CountMessage.destroy
    )
    return RaisingCallable[Tuple[JsValue], NoneType](
        environment, CountMessage.invoke
    )
...
def main() raises:
    ...
    set_environment_data(
        JsValue(JsString("mode")), JsValue(JsString("fixture"))
    )
    var worker = worker_new(
        "worker-fixture", WorkerOptions(worker_data=JsValue(42.0))
    )
    var count = Location(0)
    var code = Location(-1.0)
    _ = worker.on_callable1(JsValue(JsString("message")), listener(count, code))
    _ = worker.on_callable1(
        JsValue(JsString("exit")), listener(count, code, True)
    )
