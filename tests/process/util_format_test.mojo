...
def main() raises:
    ...
    assert_equal(format([JsValue(JsString("%% %s"))]), "%% %s")
    assert_equal(
        format(
            [
    ...
    assert_equal(
        format([JsValue(JsString("%s:%s")), JsValue(JsString("x"))]),
        "x:%s",
    )
    assert_equal(
        format(
    ...
    assert_equal(
        format([JsValue(JsString("%s %O %o")), object, object, object]),
        (
    ...
    builder.set_aggregate_children(root, [root])
    assert_equal(
        format([JsValue(JsString("%j")), builder.value(root)]),
        "[Circular]",
    )
    var rejected = False
    try:
        _ = format([JsValue(JsString("%j")), integer])
    except:
        rejected = True
    ...
    try:
        _ = format(
            [
                JsValue(JsString("%s")),
                JsValue(string_from_char_code([55296.0])),
            ]
        )
    except:
        rejected = True
