...
def retained_bytes() raises:
    ...
    bytes.set(2, 67)
    assert_equal(require_buffer(retained_alias.read_sized(1.0)).to_string(), "C")
    ...
    assert_equal(require_buffer(retained_alias.read_sized(3.0)).to_string(), "ijk")
...
def file_sizes(root: String) raises:
    ...
    assert_equal(require_buffer(retained_alias.read_sized(3.0)).to_string(), "def")
