...
def main() raises:
    ...
    assert_equal(buffer.write("a😀z", 0, 5), 5)
    assert_equal(buffer.to_string("utf8", 0, 5), "a😀")
    assert_equal(buffer.write("💩", 0, 3, "utf16le"), 2)
    assert_equal(buffer.to_string("hex", 0, 2), "3dd8")
    assert_equal(buffer.write_encoded("aabbz1", "hex"), 2)
    assert_equal(buffer.to_string("hex", 0, 2), "aabb")
    assert_equal(buffer_alloc_string(5, "abc").to_string(), "abcab")
    _ = buffer.fill_string("", 0, 8)
    ...
    assert_equal(sequence.find_string("ababa", 1), 2)
    assert_equal(sequence.last_string("ababa"), 4)
    assert_equal(sequence.last_string("ababa", 3), 2)
    assert_equal(sequence.find_string("", 100), 9)
    assert_equal(sequence.last_string("", -100), 0)
    assert_equal(sequence.last_string("a", -100), -1)
    assert_equal(sequence.find_number(353), 0)
    assert_equal(sequence.find_number(98, -2), 7)
    assert_true(sequence.includes_buffer(Buffer.from_string("bab")))
    var wide = buffer_from_string_encoded("ababa", "utf16le")
    assert_equal(wide.find_string("ba", 0, "utf16le"), 2)
    assert_equal(wide.last_string("ba", None, "utf16le"), 6)
    assert_equal(wide.find_string("ba", 3, "utf16le"), 2)
    ...
    assert_equal(buffer_concat(buffers, 3).to_string(), "abc")
    assert_equal(buffer_concat(buffers, 6).to_string("hex"), "616263640000")
